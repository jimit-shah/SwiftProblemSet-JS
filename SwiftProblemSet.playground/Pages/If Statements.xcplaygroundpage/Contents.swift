...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if bugs < 10 && hasMusic && numberOfLevels > 5 {
        ...
        print("At least \(bugs - 9) more bug(s) needs to be fixed.")
    } else if !hasMusic {
        print("Needs Music to release.")
    } else if numberOfLevels <= 5 {
        print("At least \(6 - numberOfLevels) more level(s) needed to release.")
    } else {
        print("At least \(bugs - 9) more bug(s) needs to be fixed, needs Music, and \(6 - numberOfLevels) more level(s) needed to release.")
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    if bike && run {
        ...
        print("\(name) can finish Run but can not finish Bike.")
    } else if bike {
        print("\(name) can finish Bike but can not finish Run.")
    } else {
        print("\(name) can not finish either Bike or Run.")
    }
}
